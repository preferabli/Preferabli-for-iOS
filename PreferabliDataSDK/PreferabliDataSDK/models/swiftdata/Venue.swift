...
public final class Venue: HasIntID, HasTimestamps {
    ...
    public func getFormattedAddress(one_line : Bool) -> String {
        var newLine = "\n"
        if one_line { newLine = " | " }
        let firstTwo = (PreferabliTools.isNullOrWhitespace(string: address_l1) ? "" : (address_l1! + newLine))
                    + (PreferabliTools.isNullOrWhitespace(string: address_l2) ? "" : (address_l2! + newLine))
    ...
    static public func sortVenuesByDistance(venues: [Venue], ascending : Bool, your_lat: NSNumber, your_lon: NSNumber) -> [Venue] {
        return venues.sorted {
            ...
            if d0 == d1 { return PreferabliTools.alphaSortIgnoreThe(x: $0.name ?? "", y: $1.name ?? "") }
