...
public enum Storage {
    ...
    // Sync variant
    @inline(__always)
    internal static func withContext<T>(_ body: @MainActor (ModelContext) throws -> T) rethrows -> T {
        try body(ModelContext(container))
    }
    
    // Async variant
    @inline(__always)
    internal static func withContext<T>(_ body: @MainActor (ModelContext) async throws -> T) async rethrows -> T {
        try await body(ModelContext(container))
    }
    
    // still updates UI since it is from the same container
    public static func withBackgroundContext(priority: TaskPriority = .userInitiated, _ body: @escaping @Sendable (ModelContext) throws -> Void) async throws {
    ...
    nonisolated internal static func isKeyPresentInKeyStore(key: String) -> Bool {
        return Storage.getKeyStore().object(forKey: key) != nil
    }
    
    nonisolated internal static func getKeyStore() -> UserDefaults {
