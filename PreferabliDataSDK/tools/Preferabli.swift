...
public class Preferabli {
    ...
    private func createAnonymousSession(create_anonymous_user : Bool) async throws {
        if (Storage.getKeyStore().string(forKey: "access_token").isEmptyOrWhitespace) {
            let sessionParameters =  ["login_as_anonymous" : true]
            var sessionResponse = try await api.getAlamo(requiresAccessToken: false).post(APIEndpoints.postSession, json: sessionParameters)
            sessionResponse = try await APIService.continueOrThrowPreferabliException(response: sessionResponse)
            let sessionData = SessionData(map: try APIService.continueOrThrowJSONException(data: sessionResponse.data!) as! [String : Any])
            await sessionData.saveSession()
            
            if (create_anonymous_user) {
                let createParameters: SParams = ["anonymous": true]
                var userResponse = try await api.getAlamo().post(APIEndpoints.users, json: createParameters)
    ...
    private func getIntegration() async throws {
        if (!isInternal()) {
            do {
                ...
                var integrationResponse = try await api.getAlamo().get(APIEndpoints.integration(id: integration_id))
                integrationResponse = try await APIService.continueOrThrowPreferabliException(response: integrationResponse)
                _ = try APIService.continueOrThrowJSONException(data: integrationResponse.data!) as! [String : Any]
            } catch {
    ...
    private func handleError(error : Error, onFailure: @escaping (PreferabliException) -> Void = { _ in }) {
    ...
    public func lttt(product_id : Int, year : Int = Variant.CURRENT_VARIANT_YEAR, collection_id : Int = Preferabli.PRIMARY_INVENTORY_ID, include_merchant_links: Bool = true) async throws -> [Int]
    {
        do {
            ...
            var resp = try await api.getAlamo().get(APIEndpoints.lttt, sparams: params)
    ...
    //                self.createTagActual(product_id: product_id, year: year, collection_id: Storage.getKeyStore().integer(forKey: "ratings_id"), value: rating.getValue(), tag_type: .RATING, location: location, notes: notes, price: price, quantity: quantity, format_ml: format_ml, onCompletion: onCompletion, onFailure: onFailure)
    //        })
    //    }
    //
    ...
    //        PreferabliTools.startNewAsyncWorkThread(priority: .veryHigh, {
    //                self.createTagActual(product_id: product_id, year: year, collection_id: Storage.getKeyStore().integer(forKey: "wishlist_id"), value: nil, tag_type: .WISHLIST, location: location, notes: notes, price: price, quantity: quantity, format_ml: format_ml, onCompletion: onCompletion, onFailure: onFailure)
    ...
    //                if (tag_type == .RATING) {
    //                    collection_id = Storage.getKeyStore().integer(forKey: "ratings_id")
    //                } else if (tag_type == .WISHLIST) {
    //                    collection_id = Storage.getKeyStore().integer(forKey: "wishlist_id")
    //                } else {
    //                    return
    //                }
